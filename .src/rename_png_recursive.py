...
def rename_png_files_recursive(root_path, recursive=False, prefix="", start_number=1):
    """
    Rename PNG files so that the latest (by creation time) gets the highest number,
    and the earliest file gets number 1.
    Args:
    ...
    # Keep the order as is (earliest first, latest last)
    # This way: earliest file gets 1, latest file gets highest number
    
    total_files = len(files_with_times)
    print(f"Found {total_files} PNG files in {root_path}")
    
    # Rename files with normal numbering (earliest gets 1, latest gets highest)
    for i, (file_path, creation_time) in enumerate(files_with_times):
        # Calculate the new number (earliest file gets 1, latest gets highest)
        new_number = i + 1
...
def main():
    parser = argparse.ArgumentParser(description="Rename PNG files by creation timestamp (earliest is 1, etc.)")
    ...
    if args.dry_run:
        ...
        print("Would number files so that the earliest is 1, next is 2, etc.")
        return

    rename_png_files_recursive(args.path, args.recursive, args.prefix)
